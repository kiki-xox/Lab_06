...
```{r}
diamonds %>%
  group_by(length_mm, width_mm, depth_mm) %>%
  summarise(freq = n())     #Checking for how many diamonds have and dimension of 0

DiamondsDimensions <- diamonds %>%
  filter(length_mm > 0.00) %>%
  filter(width_mm > 0.00) %>%
  filter(depth_mm > 0.00)
  
DiamondsDimensions %>%
  group_by(length_mm, width_mm, depth_mm) %>%
  summarise(freq = n())     #No longer any dimensions with the value 0
